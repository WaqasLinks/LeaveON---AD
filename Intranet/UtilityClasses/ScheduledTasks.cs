            ...
            using (var context = new PrincipalContext(ContextType.Domain, "intechww.com"))// "tenf.loc"))
            {
                ...
                using (var searcher = new PrincipalSearcher(new UserPrincipal(context)))
                {
                    ...
                    foreach (var result in AllIntechUsers)
                    {
                        ...
                        DateTime DateMarker = DateTime.ParseExact("01/01/2019", "dd/MM/yyyy", CultureInfo.InvariantCulture);
                        TimeSpan TimeDifference = WhenCreated - DateMarker;
                        ...
                        AspNetUser aspNetUser = db.AspNetUsers.FirstOrDefault(x => x.UserName == userPrincipalName_AD);
                        if (aspNetUser == null)
                        {//Insert
