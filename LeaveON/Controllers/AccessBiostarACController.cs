...
namespace LeaveON.Controllers
{
  public class AccessBiostarACController : Controller
  {
    ...
    private Task<List<Attendance>> ConnectToDBandReturnWorkingHours(string ReqMonthYear, string UserId)
    {
      ...
      for (int j = 0; j <= rowsCount - 1; j++)
      {
        ...
        {//its mean new date started. so add all previois date calcuation here and add to list
          ...
          attendance = new Attendance() { EmployeeName = "", EmployeeNumber = UserId, Date = FirsTimeIn.Date, Day = FirsTimeIn.DayOfWeek.ToString(), TimeIn = FirsTimeIn, TimeOut = LastTimeOut, WorkingHours = ThidDayWorkingHours, TotalTime = (LastTimeOut - FirsTimeIn) };
      ...
      //{
      //  if (i+1 <= rowsCount && (int)dt.Rows[i]["TNAKEY"] == 1 && (int)dt.Rows[i+1]["TNAKEY"] == 2)
      //  {
      //    dateTimeIn = (DateTime)dt.Rows[i]["SRVDT"];
      //    dateTimeOut = (DateTime)dt.Rows[i + 1]["SRVDT"];
      //    attendance = new Attendance() {EmployeeName="",EmployeeNumber=0,Date= dateTimeIn.Date, Day=dateTimeIn.DayOfWeek.ToString(), TimeIn=dateTimeIn.TimeOfDay.ToString(),TimeOut = dateTimeOut.TimeOfDay.ToString(), WorkingHours=(dateTimeOut-dateTimeIn).TotalHours.ToString(), TotalTime="" };
      //  }
      //}
    ...
    public async Task<ActionResult> ConnectToDBandReturnOffHours(string ReqMonthYear, string UserId, string reqDate)
    {
    ...
    public async Task<ActionResult> UserData(string ReqMonthYear, string UserId)
    {
      ...
      if (!string.IsNullOrEmpty(ReqMonthYear))
      {
        ...
        LstAttendances = await ConnectToDBandReturnWorkingHours(ReqMonthYearFormated, UserId);
        ...
        if (users[0].UserLeavePolicyId != null)
        {
          ...
          foreach (Leave leave in thisMonthsLeaves)
          {
            for (int i = 0; i < leave.TotalDays; i++)
            {
              Attendance accessTime_Data = new Attendance
              {
                ...
                EmployeeNumber = leave.AspNetUser.BioStarEmpNum.ToString(),
          ...
          foreach (AnnualOffDay annualOffDay in dbLeaveOn.AnnualOffDays.Where(x => x.OffDay.Value.Month == reqDate.Month && x.OffDay.Value.Year == reqDate.Year && x.UserLeavePolicyId == UserLeavePolicyId).ToList<AnnualOffDay>())
          {
            ...
            {
              EmployeeName = biostarEmpName,
              EmployeeNumber = iEmpNum.ToString(),
    ...
    public async Task<ActionResult> UserDataDetail(string ReqMonthYear, string UserId)
    {
      ViewBag.MonthSelectList = GetMonthSelectList();
      //ViewBag.SelectedMonth = monthSelectList[0];
      DateTime reqDate;
      ...
      if (!string.IsNullOrEmpty(ReqMonthYear))
      {
        ...
        //{
        string ReqMonthYearFormated = reqDate.Month.ToString("00") + "-" + reqDate.Year;
        //LstAttendances = await ConnectToDBandReturnOffHours(ReqMonthYearFormated, UserId, "");
        //empData = dbBioStar.UD_TB_AccessTime_Data.Where(x => userIds.Contains(x.EmployeeNumber.Value) && ((x.Date_IN.Value.Month == reqDate.Month && x.Date_IN.Value.Year == reqDate.Year) ||
        //x.Date_OUT.Value.Month == reqDate.Month && x.Date_OUT.Value.Year == reqDate.Year)).AsQueryable<UD_TB_AccessTime_Data>();
        //var groupByData = empData.ToList().GroupBy(x => x.Date_IN.Value.Day);
        //}
